...
class ValidationOrchestrator {
  ...
  async validate(prData) {
    ...
    try {
      ...
      // 1. Validate PR title
      const titleValidation = await validationService.validateTitle(prData.title);
      ...
      if (!titleValidation.isValid) resultManager.addError(titleValidation.error);

      // 2. Validate PR description
      const descriptionValidation = validationService.validatePRDescription(prData.body);
      if (!descriptionValidation.isValid) resultManager.addError(descriptionValidation.error);

      // 3. Validate file count
      const fileCountValidation = validationService.validateFileCount(prData.files);
      ...
      if (prData.files.length > 0) {
        ...
        if (titleValidation.isValid && filePathValidation.isValid) {
          if (titleValidation.actionType === 'Remove') {
            const removeValidation = await validationService.validateRemoveOperation(file, titleValidation);
            if (!removeValidation.isValid) resultManager.addError(removeValidation.error);
          ...
          const consistencyValidation = validationService.validateTitleFileConsistency(titleValidation, file.filename);
          if (!consistencyValidation.isValid) resultManager.addError(consistencyValidation.error);
        }
      } else {
      ...
      const report = await reportGenerator.generateValidationReport(resultManager.getResult(), prData.author);
