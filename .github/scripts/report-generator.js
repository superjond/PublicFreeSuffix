...
function categorizeErrors(errors) {
  ...
  errors.forEach(error => {
    ...
    if (errorLower.includes('title format') || errorLower.includes('pr title')) categories.titleFormat = true;
    if (errorLower.includes('description') && errorLower.includes('1300')) categories.descriptionLength = true;
    if (errorLower.includes('file change') || errorLower.includes('file count')) categories.fileCount = true;
    if (errorLower.includes('file path') || errorLower.includes('whois/')) categories.filePath = true;
    if (errorLower.includes('json format') || (errorLower.includes('json') && errorLower.includes('invalid'))) categories.jsonFormat = true;
    if (errorLower.includes('registrant') || errorLower.includes('email')) categories.registrant = true;
    if (errorLower.includes('domain') && !errorLower.includes('reserved')) categories.domain = true;
    if (errorLower.includes('reserved') || errorLower.includes('conflicts')) categories.reservedWords = true;
    if (errorLower.includes('sld') || errorLower.includes('suffix')) categories.sld = true;
    if (errorLower.includes('nameservers') || errorLower.includes('dns')) categories.nameservers = true;
    if (errorLower.includes('agree_to_agreements') || errorLower.includes('agreement')) categories.agreements = true;
    if (errorLower.includes('not match') || errorLower.includes('consistency')) categories.consistency = true;
    if (errorLower.includes('remove operation') || errorLower.includes('cannot remove file') || (errorLower.includes('file') && errorLower.includes('removed'))) categories.removeOperation = true;
  });
...
async function generateValidationReport(validationResult, mentionUser = null) {
  ...
  try {
    if (process.env.PR_NUMBER) {
      ...
      const labels = validationResult.isValid ? ['validation-passed'] : ['validation-failed'];
      await githubService.updatePullRequestLabels(process.env.PR_NUMBER, labels, owner, repo);
    }
  } catch (error) {
